...
class AttentionSeq2SeqModel(object):
    ...
    def __init__(self, model_path=None, *, src_vocab_size=None, tgt_time_step=None, tgt_vocab_size=None,
                 start_token_id=None, end_toekn_id=None, attention_mechanism="normed_bahdanau", batch_size=None):
        # 1.如果传入模型路径，尝试加载模型
        if model_path is not None:
            ...
            return
        # 2检测训练参数
        if src_vocab_size is None or tgt_time_step is None or tgt_vocab_size is None \
                or start_token_id is None or end_toekn_id is None:
    ...
    def build_input_tensor_inference(self):
        ...
        with tf.variable_scope("input_tensor_inference"):
            ...
            self.start_tokens = tf.tile(tf.constant(value=self.start_token_id, dtype=tf.int32, shape=[1]),
                                        multiples=self.inference_batch_size, name="start_tokens")
    ...
    def build_decoder(self, encoder_states, encoder_outputs, decoder_rnn_layer_size, decoder_num_units,
        ...
        with tf.variable_scope("decoder"):
            ...
            # 加入attention
            self.decoder_rnn_layer_attention = self.build_rnn_layer_attention(encoder_outputs, self.decoder_rnn_layer,
                                                                              decoder_num_units, self.src_batch_seq_len)
    ...
    # 构造推理时的decoder
    def build_decoder_inference(self, encoder_outputs, encoder_states, beam_width, decoder_num_units,
                                decoder_rnn_layer_size, decoder_cell_type, decoding_method):
        ...
        with tf.variable_scope("decoder"):
            ...
            if decoding_method == 'beamsearch':
                ...
                inference_rnn_layer_attention = self.build_rnn_layer_attention(memory, rnn_layer, decoder_num_units,
                                                                               src_seq_len)
            ...
            # greedy解码
                ...
                rnn_layer_attention = self.build_rnn_layer_attention(encoder_outputs, rnn_layer, decoder_num_units,
                ...
                raise Exception(
                    '不支持的解码方法，只可选 greedy 或者 beam search/Unsupported decoding method, only greedy or beamsearch are allowed!')
        # Dynamic decoding
    ...
    # 构建训练计算图和推理计算图
    # 解决attention参数共享问题
    def build_graph(self, src_embedding_size, tgt_embedding_size, is_encoder_bidirectional=False, rnn_layer_size=2,
                    rnn_num_units=128, cell_type="LSTM", lr=0.001, decoding_method="greedy", beam_width=10):
        # 构造计算图
        ...
        with inference_graph.as_default():
            ...
            # 选择合适的解码方法
            self.build_decoder_inference(encoder_outputs, encoder_states, beam_width, rnn_num_units, rnn_layer_size,
                                         cell_type, decoding_method)
            ...
            with train_graph.as_default():
                ...
                if is_encoder_bidirectional:
                    # 需要注意使用bidirectional，encoder rnn的num_units变为decoder的一半，这是为了能够保证encoder_states和decoder的输入shape能对应上
                    encoder_outputs, encoder_states = self.build_encoder_bi(rnn_layer_size, rnn_num_units / 2,
                                                                            cell_type)
                else:
                    encoder_outputs, encoder_states = self.build_encoder(rnn_layer_size, rnn_num_units, cell_type)
                decoder_outputs = self.build_decoder(encoder_states, encoder_outputs, rnn_layer_size, rnn_num_units,
                                                     cell_type)
    ...
    # 训练
    def train(self, model_path, generator, src_embedding_size, tgt_embedding_size, is_encoder_bidirectional=False,
              rnn_layer_size=2, rnn_num_units=128, cell_type="LSTM", lr=0.001, decoding_method="greedy", beam_width=10):
        train_graph, inference_graph = self.build_graph(src_embedding_size, tgt_embedding_size,
                                                        is_encoder_bidirectional, rnn_layer_size, rnn_num_units,
                                                        cell_type, lr, decoding_method, beam_width)
